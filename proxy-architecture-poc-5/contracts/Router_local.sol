    ...
    fallback() external {
        ...
        assembly {
            ...
            function findImplementation(sig) -> result {
                let UpgradeModule := 0x01c1DeF3b91672704716159C9041Aeca392DdFfb
                let DummyModule := 0xF8e31cb472bc70500f08Cd84917E5A1912Ec8397
                let AnotherDummyModule := 0xc0F115A19107322cFBf1cDBC7ea011C19EbDB4F8
                let YetAnotherDummyModule := 0xc96304e3c037f81dA488ed9dEa1D8F2a48278a75
                let SettingsModule := 0x34B40BA116d5Dec75548a9e9A8f15411461E8c70
                let OwnerModule := 0xD42912755319665397FF090fBB63B1a31aE87Cee
                let AModule := 0x07882Ae1ecB7429a84f1D53048d35c4bB2056877
                let BModule := 0x22753E4264FDDc6181dc7cce468904A80a363E44
                let RegistryModule := 0xA7c59f010700930003b33aB25a7a0679C860f29c
                ...
                if lt(sig,0x7fb68261) {
                    if lt(sig,0x3df86c93) {
                        if lt(sig,0x1fa85301) {
                            ...
                            if lt(sig,0x1b9255bb) {
                                if lt(sig,0x1990b834) {
                                    switch sig
                        ...
                        if lt(sig,0x2fd9abb4) {
                            if lt(sig,0x274b46ef) {
                                if lt(sig,0x245ea161) {
                                    if lt(sig,0x2268b1a6) {
                                        switch sig
                            ...
                            if lt(sig,0x2c22938e) {
                                if lt(sig,0x28e805a0) {
                                    switch sig
                        ...
                        if lt(sig,0x37e5d945) {
                            if lt(sig,0x34dfe75e) {
                                if lt(sig,0x319ceddf) {
                                    switch sig
                        ...
                        if lt(sig,0x3b75b5fb) {
                            if lt(sig,0x38b56acb) {
                                switch sig
                    ...
                    if lt(sig,0x5e54e982) {
                        ...
                        if lt(sig,0x581c2eaf) {
                            if lt(sig,0x52fb8013) {
                                if lt(sig,0x50fe6976) {
                                    switch sig
                    ...
                    if lt(sig,0x6dc87a90) {
                        ...
                        if lt(sig,0x6bc20481) {
                            if lt(sig,0x69b012fb) {
                                switch sig
                    ...
                    if lt(sig,0x76e2561e) {
                        if lt(sig,0x746831ee) {
                            if lt(sig,0x719fcbbe) {
                                switch sig
                ...
                if lt(sig,0xc224a44a) {
                    if lt(sig,0xa2b5080c) {
                    ...
                    if lt(sig,0xb23d404c) {
                        if lt(sig,0xa88326ba) {
                            if lt(sig,0xa5fd5db5) {
                                if lt(sig,0xa45ee043) {
                                    switch sig
                        ...
                        if lt(sig,0xadfb8d35) {
                            if lt(sig,0xab7adb1d) {
                                switch sig
                    ...
                    if lt(sig,0xb9f6c7aa) {
                        if lt(sig,0xb5a44554) {
                    ...
                    if lt(sig,0xbede07ca) {
                        if lt(sig,0xbc7e7e1d) {
                            switch sig
                ...
                if lt(sig,0xde05552d) {
                    if lt(sig,0xcf1848a0) {
                        if lt(sig,0xc84fa477) {
                            if lt(sig,0xc43c0a65) {
                                if lt(sig,0xc3c9313c) {
                                    switch sig
